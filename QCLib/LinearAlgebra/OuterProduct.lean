...
variable {n m : Type*}
variable {𝕜 : Type*} [RCLike 𝕜] (v v' : EuclideanSpace 𝕜 n) (w w' : EuclideanSpace 𝕜 m)

noncomputable instance :
    OuterProduct (EuclideanSpace 𝕜 n) (EuclideanSpace 𝕜 m) (EuclideanSpace 𝕜 (n × m)) where
  tprod v w := (equiv (n × m) 𝕜).symm (equiv _ _ v ⨂ equiv _ _ w)

@[simp]
theorem outerProduct_apply {i j} : (v ⨂ w) (i, j) = (v i) * (w j) := by
  simp [OuterProduct.tprod]

@[simp]
theorem zero_outerProduct : (0 : EuclideanSpace 𝕜 n) ⨂ w = 0 := by
  ext ⟨i, j⟩; simp

@[simp]
theorem outerProduct_zero : v ⨂ (0 : EuclideanSpace 𝕜 m) = 0 := by
  ext ⟨i, j⟩; simp

@[simp]
theorem add_outerProduct : (v + v') ⨂ w = (v ⨂ w) + (v' ⨂ w) := by
  ext ⟨i, j⟩; simp [add_mul]

@[simp]
theorem outerProduct_add : v ⨂ (w + w') = (v ⨂ w) + (v ⨂ w') := by
  ext ⟨i, j⟩; simp [mul_add]

theorem sum_outerProduct {ι : Type*} (S : Finset ι) (f : ι → EuclideanSpace 𝕜 n) :
    (∑ i ∈ S, f i) ⨂ w = ∑ i ∈ S, f i ⨂ w := by
  induction S using Finset.cons_induction <;> simp_all

theorem outerProduct_sum {ι : Type*} (S : Finset ι) (f : ι → EuclideanSpace 𝕜 m) :
    v ⨂ (∑ i ∈ S, f i) = ∑ i ∈ S, v ⨂ f i := by
  induction S using Finset.cons_induction <;> simp_all

@[simp]
theorem smul_outerProduct (c : 𝕜) : (c • v) ⨂ w = c • (v ⨂ w) := by
  ext ⟨i, j⟩; simp [smul_eq_mul, mul_assoc]

@[simp]
theorem outerProduct_smul (c : 𝕜) : v ⨂ (c • w) = c • (v ⨂ w) := by
  ext ⟨i, j⟩; simp [smul_eq_mul, mul_left_comm]

@[simp]
theorem outerProduct_smul_smul (c d : 𝕜) : (c • v) ⨂ (d • w) = (d * c) • (v ⨂ w) := by
  rw [outerProduct_smul, smul_outerProduct, smul_smul]

@[simp]
theorem neg_outerProduct : (-v) ⨂ w = -(v ⨂ w) := by
  ext ⟨i, j⟩; simp [neg_mul]

@[simp]
theorem outerProduct_neg : v ⨂ (-w) = -(v ⨂ w) := by
  ext ⟨i, j⟩; simp [mul_neg]

theorem outerProduct_left_injective (hw : w ≠ 0) :
    Function.Injective (fun v : EuclideanSpace 𝕜 n => v ⨂ w) := by
  obtain ⟨j, hj⟩ : ∃ j, w j ≠ 0 := not_forall.mp fun h => hw (by ext j; simp [h j])
  intro v v' h
  ext i
  simpa [hj] using congrArg (fun f => f (i, j)) h

theorem outerProduct_right_injective (hv : v ≠ 0) :
    Function.Injective (fun w : EuclideanSpace 𝕜 m => v ⨂ w) := by
  obtain ⟨i, hi⟩ : ∃ i, v i ≠ 0 := not_forall.mp fun h => hv (by ext i; simp [h i])
  intro w w' h
  ext j
  simpa [hi] using congrArg (fun f => f (i, j)) h

noncomputable def outerProductBilinearMap :
    EuclideanSpace 𝕜 n →ₗ[𝕜] EuclideanSpace 𝕜 m →ₗ[𝕜] EuclideanSpace 𝕜 (n × m) :=
  LinearMap.mk₂ 𝕜 (· ⨂ ·) (by simp) (by simp) (by simp) (by simp)

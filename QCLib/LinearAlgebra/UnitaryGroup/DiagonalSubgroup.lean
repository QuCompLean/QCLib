...
`VDiag : ι → (n → unitary ℂ)` of their main diagonals, and use
`noncommProd_comp_eq_prod` to rewrite
```
  Finset.noncommProd i ∈ s, V i
= Finset.noncommProd i ∈ s, diagonal (VFun i)
= diagonal Finset.prod i ∈ s, (VFun i)
```
